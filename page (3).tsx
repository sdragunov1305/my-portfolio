...
export default function AdminPage() {
  ...
  async function publish(e: FormEvent) {
    ...
    try {
      const res = await fetch("/api/vlogs", { method: "POST", headers: headers(), body: form });
  ...
  async function remove(id: string) {
    ...
    const res = await fetch(`/api/vlogs?id=${encodeURIComponent(id)}`, {
      method: "DELETE",
      headers: headers(),
    });
    ...
    <main className="min-h-screen bg-[#120b25] px-4 py-10 text-white">
      <div className="mx-auto w-full max-w-xl">
        ...
        <p className="mt-8 text-center text-xs text-white/35">
          <a href="/" className="underline decoration-white/20 hover:text-white/70">
            Back to site
          </a>
        </p>
      </div>
